...
async def get_messages(room_id: str):
    res = supabase.table("chat_messages").select("*").eq("room_id", room_id).order("created_at", asc=True).execute()
...
async def chat_endpoint(req: ChatRequest):
    ...
    history_res = supabase.table("chat_messages").select("sender, content").eq("room_id", room_id).order("created_at", asc=True).execute()
